...
// Challenge 1:
app.post('/incoming/call', function(request, response) {
    var twiml = new twilio.TwimlResponse();
    twiml.say('You are learning node.js! Good for you.');
    twiml.say('Welcome to your personal concierge service!')
        .gather({
            numDigits: 1,
            action:'/handle-key'
        }, function() {
...
app.get('/incoming/sms', function(request, response) {
    var twiml = new twilio.TwimlResponse();
    twiml.sms('You can do node. Look at you go!');

    response.set('Content-Type', 'text/xml');
    response.send(twiml.toString());
});

app.post('/handle-key', function(request, response) {
    var twiml = new twilio.TwimlResponse();
    twiml.say('Goodbye!', {language: 'en-gb'});
});
